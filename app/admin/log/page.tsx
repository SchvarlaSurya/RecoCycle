  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-stone-200 bg-white p-4">
        ...
        <div>
          <p className="text-sm font-medium text-stone-900">Catatan audit tidak dapat dihapus</p>
          <p className="mt-0.5 text-xs text-stone-500">Semua aksi admin secara otomatis tercatat untuk mencegah kecurangan internal dan menjaga transparansi operasional.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-stone-200 bg-white py-2.5 pl-10 pr-4 text-sm text-stone-900 outline-none placeholder:text-stone-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30"
          />
        </div>
        <select
          value={filterAction}
          onChange={(event) => setFilterAction(event.target.value as "all" | ActivityAction)}
          className="rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm text-stone-900 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30"
        >
      ...
      <p className="text-xs text-stone-500">{filtered.length} catatan ditemukan</p>
      ...
      <div className="space-y-3">
        {filtered.map((log) => (
          <div
            key={log.id}
            className="rounded-2xl border border-stone-200 bg-white p-4 transition-all hover:border-emerald-300 hover:shadow-sm"
          >
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-stone-500">oleh</span>
                  ...
                  <p className="mt-0.5 truncate text-xs text-stone-500">{log.details}</p>
                </div>
              </div>
              <div className="flex-shrink-0 text-right">
                <p className="text-xs text-stone-500">{formatTimestamp(log.createdAt)}</p>
                <p className="mt-0.5 font-mono text-[10px] text-stone-400">ID: {log.id}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        {filtered.length === 0 && (
          <div className="rounded-2xl border border-stone-200 bg-white p-8 text-center">
            <p className="text-stone-500">Tidak ada log aktivitas ditemukan.</p>
          </div>
        )}
      </div>
    </div>
