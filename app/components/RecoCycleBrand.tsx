...
function RecoCycleIcon({ theme = "light", size = "md" }: { theme?: "light" | "dark"; size?: "sm" | "md" | "lg" }) {
  const sizes = {
    sm: "h-10 w-10",
    md: "h-12 w-12",
    lg: "h-14 w-14",
  ...
  return (
    <span
      className={clsx(
        "relative inline-flex items-center justify-center overflow-hidden rounded-[1.35rem] border shadow-[0_16px_36px_rgba(5,150,105,0.22)]",
        theme === "dark"
          ? "border-white/10 bg-white/8 backdrop-blur-xl"
          : "border-white/70 bg-white/80 backdrop-blur-xl",
        sizes[size]
      )}
      aria-hidden="true"
    >
      <svg viewBox="0 0 64 64" className="h-[85%] w-[85%]" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="rc-wheel" x1="12" y1="14" x2="43" y2="43" gradientUnits="userSpaceOnUse">
        ...
        <path d="M44.7 18.7l.7 7.4-7.1-2.1 2.2-1.4a13.5 13.5 0 0 0-8.6-3.3c-6.8 0-12.5 5.2-13.2 12h-3.8c.8-8.9 8.3-15.8 17-15.8 4.3 0 8.1 1.4 11.1 3.8l1.7-.6Z" fill="url(#rc-wheel)" />
        <path d="M49.8 34.2a17 17 0 0 1-8.4 12.2" stroke={stroke} strokeWidth="2.8" strokeLinecap="round" opacity="0.3" />
        <path d="M41.4 46.4l-6.7-3.2 5.5-5 0 2.6a13.5 13.5 0 0 0 6-9.4c.7-6.8-3.7-13.2-10.3-15.2l.9-3.7c8.6 2.7 14.3 11.2 13.4 20-.4 4.3-2.2 7.9-4.9 10.7l.4 1.9Z" fill="url(#rc-wheel)" />
        <path d="M14.6 37.7a17 17 0 0 1 6-13.5" stroke={stroke} strokeWidth="2.8" strokeLinecap="round" opacity="0.3" />
        <path d="M20.6 24.2l6 4.3-6.2 4.1.2-2.6a13.5 13.5 0 0 0 2.8 17.3c5.3 4.4 13 4.8 18.6 1.3l2 3.3c-7.2 4.6-16.8 4.2-23.4-1.2-3.2-2.6-5.3-5.9-6.3-9.7l-1.7-1.1Z" fill="url(#rc-wheel)" />
...
export default function RecoCycleBrand({
  theme = "light",
  size = "md",
  showTagline = false,
  className,
  ...
  return (
    <div className={clsx("flex items-center gap-3", className)}>
      <RecoCycleIcon theme={theme} size={size} />
      <div>
        <span className={clsx("block font-semibold tracking-wide", theme === "dark" ? "text-white" : "text-slate-900")}>
          RecoCycle
        </span>
        {showTagline && (
          <span
